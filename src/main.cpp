...
#include "../BasicIO/BasicIO.hpp"
#include "../GeomImage/GeomImage.hpp"
...
void Home()
{
    ...
    cout << "Press 1 for Jarvis' March (gift wrapping) algorithm (with O(n h) complexity)." << endl;
    cout << "Press 2 for Graham's Scan algorithm (with O(n logn) complexity)." << endl;
    ...
    else if(opt == "2")
    {
        ...
        cerr << endl << "ERROR: No Such Option available" << endl;
        Home();
    }
}
...
void ConvexHullGraham()
{
    ...
    {
        if(pol.getPoint(i).isSame(start))
            break;
